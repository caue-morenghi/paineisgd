import { Box, Checkbox, Typography, IconButton, Snackbar, Alert } from "@mui/material";
...
export const FormBD = () => {
  ...
  const handleDataSubmit = async (data: TBDForm) => {
    ...
    if (!documentType) { 
      setSnackbarMessage("Por favor, selecione o tipo de documento (CPF ou CNPJ).");
    ...
    try {
      ...
      if (resposta.includes("Duplicate entry")) {
        setSnackbarMessage("Erro ao inserir dados: Banco já foi cadastrado!");
        setSnackbarSeverity("warning");
        setSnackbarOpen(true);
      }
      if (resposta.includes("Credenciais salvas com sucesso!")) {
        setSnackbarMessage("Credenciais salvas com sucesso!");
      ...
      if (resposta.includes("Erro ao conectar ao banco de dados")) {
        setSnackbarMessage("Erro ao conectar ao banco de dados: verifique as credenciais e tente novamente.");
